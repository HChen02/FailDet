...
shipped in the bundle (~14 GB for RLBench alone). Two options:

- **Generate locally**: copy `experiments/preprocess_da3.py` from the
  parent repo into this bundle, then run:
  ```bash
  python experiments/preprocess_da3.py --splits rlbench_train rlbench_test
  ```
  Takes ~10 min on a 5090 / A100.
- **Skip DA3 methods**: easier — see step 2.
