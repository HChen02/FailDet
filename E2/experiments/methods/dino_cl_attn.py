"""DINOv2-CL-Attn: pure-vision supervised contrastive learning with
**attention pooling** over the per-image CLS tokens.

The contrastive twin of DINOv2-CE-Attn, and the attention-pool twin of
DINOv2-CL. Tests the 2x2 design space (CE vs CL) × (mean pool vs attn
pool) on the same DINOv2-L backbone.

Architecture
------------
    inputs : N viewpoint PIL images per sample (RLBench=8, UR5=6, BDV2=2).
    ...
    pool   : trainable AttentionPool — a single learnable query attends
             ...
             followed by LayerNorm. Replaces the mean-pool of DINOv2-CL.
...
informative for the contrastive geometry per sample, rather than
weighting all viewpoints equally. Attention weights are captured at
test inference and dumped to metrics.json (mean per image, mean per
class, full per-sample matrix).

Variants (config["variant"])
----------------------------
    frozen - DINOv2 weights frozen. Per-image CLS tokens for train + val
             are extracted once and cached as [N, n_images, 1024].
             AttentionPool + projection head train against the cache.
             lr_proj=1e-2, epochs=30, batch_size=32 (or 512), temperature=1.0.
    lora   - LoRA r=16 on DINOv2 attention + train AttentionPool + proj.
             lr_encoder=1e-4, lr_proj=1e-2, epochs=15, batch_size=32.

Why this method matters
-----------------------
- DINOv2-CL vs DINOv2-CL-Attn directly tests whether attention pooling
  helps the CL objective the same way it helped CE (the E1 result showed
  CE-Attn beat CE on multiclass by +3-4 pp F1).
- DINOv2-CE-Attn vs DINOv2-CL-Attn isolates "CE vs CL" with everything
  else held constant (encoder, pool, head architecture).
"""
...
def _build_attention_pool(dim: int = 1024, n_heads: int = 4):
    """Single-query attention pool over per-image CLS tokens. Same module
    as DINOv2-CE-Attn so the 2x2 (CE-vs-CL × mean-vs-attn) ablation is
...
def train_dino_cl_attn(config: dict) -> dict:
    ...
    log = setup_run_logger(
        run_dir, name=f"dino_cl_attn.{config.get('exp_id')}.{seed}")
    metrics_path = run_dir / "metrics.json"

    log.info(f"=== train_dino_cl_attn | exp_id={config.get('exp_id')} "
             f"task={task} variant={variant} seed={seed} ===")
    ...
    if variant == "frozen":
        ...
        encoder.train(False)
        log.info("DINOv2 frozen — only AttentionPool + projection head train.")
    else:
    ...
    log.info(f"AttentionPool params={n_pool:,}  proj_head params={n_proj:,}")
    ...
    def _encode_indices(ds, indices):
        """Frozen DINOv2 forward over each sample's images; return per-image
        CLS tokens stacked as [N, n_images, 1024]. AttentionPool consumes
        this directly. Assumes uniform n_images within `indices`."""
        if not indices:
        ...
        for start in range(0, len(indices), chunk):
            ...
            for s in samples:
                if len(s["images"]) != n_images_ref:
                    raise RuntimeError(
                        f"AttentionPool requires uniform n_images per "
                        f"sample, saw {len(s['images'])} vs reference "
                        f"{n_images_ref}")
    ...
    if variant == "frozen":
        ...
        head_params = list(attn_pool.parameters()) + list(proj_head.parameters())
        optimizer = torch.optim.AdamW(
            head_params, lr=LR_PROJ, weight_decay=0.01)
    else:
        ...
        optimizer = torch.optim.AdamW(
            [
                ...
                {"params": head_params, "lr": LR_PROJ, "name": "head"},
            ],
            weight_decay=0.01,
        )
    ...
    log.info(f"Training: variant={variant} epochs={EPOCHS} bs={BATCH_SIZE} "
             ...
             f"tau={TEMPERATURE} attn_heads={ATTN_HEADS} "
    ...
    def _embed_from_features(per_image_feats, *, return_attn=False):
        """per_image_feats: [B, n_images, 1024] -> L2-normalised 128-d emb.
        Optionally returns the AttentionPool's attention weights too."""
        pooled, attn_w = attn_pool(per_image_feats)
        emb = F.normalize(proj_head(pooled), dim=-1)
        return (emb, attn_w) if return_attn else emb

    def _embed_from_images(images_list, *, return_attn=False):
        ...
        return _embed_from_features(feats, return_attn=return_attn)
    ...
    def _val_loss():
        attn_pool.train(False); proj_head.train(False)
        if variant == "lora":
            encoder.train(False)
        embeds: list = []
        all_l: list = []
        ...
        attn_pool.train(True); proj_head.train(True)
        if variant == "lora":
            encoder.train(True)
        if not embeds:
            return float("nan")
    ...
    for epoch in range(EPOCHS):
        ...
        if use_early_stopping and epochs_since_improvement >= EARLY_STOPPING_PATIENCE:
            ...
            log.info(f"  [DINOv2-CL-Attn] early stopping at epoch {early_stopped_at}")
            break
    ...
    blob["train"] = {
        ...
        "temperature": TEMPERATURE, "attn_pool_heads": ATTN_HEADS,
        "epoch_losses": epoch_losses,
        "epoch_val_losses": epoch_val_losses,
        "epoch_intra_class_sim": epoch_intra_sim,
        "epoch_inter_class_sim": epoch_inter_sim,
    ...
    with torch.no_grad():
        if variant == "frozen":
            ...
            for start in range(0, len(test_feats), BATCH_SIZE):
                ...
                e, attn_w = _embed_from_features(f_batch, return_attn=True)
                e = e.cpu()
                aw = attn_w.squeeze(1).detach().cpu().tolist()
                for vec, gt_pos, a in zip(
                        e, test_label_pos[start:start + BATCH_SIZE], aw):
            ...
            for start in range(0, len(test_indices), BATCH_SIZE):
                ...
                gt_pos_list = [cid_to_pos[
                    _remap_label_for_task(int(s["failure_label"]), task)]
                    for s in samples]
                e, attn_w = _embed_from_images(images_list, return_attn=True)
                e = e.cpu()
                aw = attn_w.squeeze(1).detach().cpu().tolist()
                for vec, gt_pos, a in zip(e, gt_pos_list, aw):
                    sims = (vec.unsqueeze(0) @ centroids.T).squeeze(0)
                    pred_pos = int(sims.argmax().item())
                    predictions.append(label_names[pred_pos])
                    ground_truths.append(label_names[int(gt_pos)])
    ...
    log.info(f"DINOv2-CL-Attn eval: acc={m['accuracy']:.4f} "
    ...
    log.info(f"  mean attention per image: "
             f"{[round(x,4) for x in attn_mean_per_image]}")
    ...
    save_results_atomically(blob, metrics_path)
    # Persist projection head + attention pool + centroids (and LoRA
    # adapter if applicable) for cross-domain eval reuse.
    head_payload = {
        "method": "dino_cl_attn",
        "variant": variant,
...
# ---------------------------------------------------------------------------
# Eval-only mode — load attn_pool + proj_head + centroids from head.pt.
# ---------------------------------------------------------------------------

def eval_dino_cl_attn(config: dict) -> dict:
    seed = int(config.get("seed", 42))
    set_seed(seed)
    task = str(config.get("task", "8class"))
    ...
    log = setup_run_logger(
        run_dir, name=f"dino_cl_attn.eval.{config.get('exp_id')}.{seed}")
    metrics_path = run_dir / "metrics.json"
    log.info(f"=== eval_dino_cl_attn | from_checkpoint={ckpt_dir} ===")
    ...
    k_present = len(present_classes)
    cid_to_pos = {c: i for i, c in enumerate(present_classes)}
    centroids = head["centroids"].to(device)
    ...
    with torch.no_grad():
        for start in range(0, len(test_feats), BATCH_SIZE):
            f_batch = test_feats[start:start + BATCH_SIZE].to(device)
            pooled, attn_w = attn_pool(f_batch)
            e = F.normalize(proj_head(pooled), dim=-1)
            aw = attn_w.squeeze(1).detach().cpu().tolist()
            for vec, gt_pos, a in zip(
                    e.cpu(), test_label_pos[start:start + BATCH_SIZE], aw):
    ...
    m = compute_classification_metrics(ground_truths, predictions, label_names)
    log.info(f"DINOv2-CL-Attn eval-only: acc={m['accuracy']:.4f} "
             f"f1_macro={m['f1_macro']:.4f}")
    if m.get("majority_class_warning"):
        log.warning(m["majority_class_warning"])
    ...
    save_results_atomically(blob, metrics_path)
    write_done_flag(run_dir, {
        "method": "dino_cl_attn", "variant": variant,
        "f1_macro": m["f1_macro"], "accuracy": m["accuracy"],
        "eval_only": True,
    })
    return blob
